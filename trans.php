...
switch ($job)
{
	case 'trans':
      ...
      if(file_exists('inc/contact.txt'))
      {
        $contact = file_get_contents('inc/contact.txt');
	...
	case 'confirm':
			...
			if($row)
			{
        file_get_contents("inc/contact.txt");
        $contact = explode(',', $contact);
        if((!empty($phone) && $phone != $contact[0]) || (!empty($email) && $email != $contact[1]))
        {
          $contact = $phone . "," . $email;
          file_put_contents("inc/contact.txt", $contact);
        }
